...
**Status:** ✅ Completed & Closed
**Files:**
- `lib/pdf.ts` - PDF generation function
- `lib/index.ts` - Updated exports
- `components/Dashboard/SummaryStats.tsx` - Added PDF button
- `messages/en.json` - English translations
- `messages/es.json` - Spanish translations
- `package.json` - Added jspdf dependency

**Features Implemented:**
- `generateGoalsPDF(goals, locale)` creates professionally formatted PDFs:
  - Header with title, generation date, summary stats
  - Goals grouped by bucket (Safety, Growth, Dream)
  - Goal cards with priority badge, title, amount, description, target date, why it matters
  - Footer with page numbers and app attribution
- Full i18n support (English/Spanish)
- Bucket colors visually represented (colored bars and badges)
- Clean, professional layout with proper spacing
- PDF button in SummaryStats with download icon
...
| Section | Content |
|---------|---------|
| Header | "My Investment Goals", date, total goals & amount |
| Bucket Sections | Colored header bar, sorted goals |
| Goal Cards | Priority badge, title, amount, description, date, why |
| Footer | Page numbers, "Generated by Investment Goals App" |
